...
const menuItems = [
  ...
  },
  {
    ...
    icon: CalendarDays,
    path: '/calendar',
    active: currentUrl.startsWith('/calendar')
  },
  {
    name: 'Usuarios',
    icon: Users,
...
<template>
  <div data-slot="sidebar-content" data-sidebar="content" :class="cn('flex-1 overflow-y-auto py-4 px-2', props.class)">
    <Link v-for="item in menuItems" :key="item.name" :href="item.path"
      ...
      <span v-if="item.active && expanded" class="w-1.5 h-1.5 rounded-full bg-green-400 ml-auto" />
    </Link>
    <slot />
  </div>
</template>

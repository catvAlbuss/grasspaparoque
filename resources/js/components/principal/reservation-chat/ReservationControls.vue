...
<template>
    <div v-if="step === 'date'" class="space-y-2">
        <label for="reservation-date" class="block text-xs font-medium text-emerald-100">Selecciona el día de tu
            reserva</label>
        <input id="reservation-date" type="date" :min="minDate" :disabled="disabled || typing"
    ...
    <div v-else-if="step === 'duration'" class="space-y-2">
        ...
        <div class="grid grid-cols-4 gap-2">
            <button v-for="hours in 8" :key="hours" type="button" :disabled="disabled || typing"
    ...
    <div v-else-if="showTextInput" class="flex gap-2">
        <input v-model="value" :disabled="disabled || typing" type="text" autocomplete="off"
            ...
            :placeholder="placeholders[step]" @keydown.enter="submit" />
        <button type="button" :disabled="disabled || typing || !value.trim()"
    ...
    <button v-if="canUpload" type="button"
        class="w-full rounded-xl bg-emerald-600 px-3 py-2.5 text-xs font-semibold text-white" @click="emit('upload')">
        Subir imagen del comprobante
    </button>

    <div v-if="asking" class="flex gap-2">
        <input v-model="question" type="text" placeholder="Escribe tu pregunta"
            class="min-w-0 flex-1 rounded-xl border border-emerald-800 bg-slate-900 px-3 py-2 text-sm text-white"
            @keydown.enter="ask" />
        <button type="button" class="rounded-xl bg-sky-600 px-3 text-xs font-semibold text-white" @click="ask">
            Preguntar
        </button>
    </div>
    <div class="flex justify-between gap-3 text-xs">
        <button type="button" class="text-sky-200 underline underline-offset-2" @click="asking = !asking">
            ¿Necesitas ayuda?
        </button>
        <button type="button" class="text-emerald-200 underline underline-offset-2" @click="emit('reset')">
            Empezar de nuevo
        </button>
    </div>
</template>

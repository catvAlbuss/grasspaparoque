...
const form = useForm({
    ...
    precio: '',
    descripcion: '',
    estado: 'free' as 'free' | 'busy',
    tipo_ambiente: 'propio' as 'compartido' | 'propio',
    ambiente_grupo: null as number | null,
});
...
// ── Estado inteligente ───────────────────────────────
const getEstadoInfo = (estado: eventos['estado']) => {
    if (estado === 'free') {
        return { label: 'Disponible', icon: CheckCircle2, badgeClass: 'inline-flex items-center gap-1 rounded-full border border-green-500/20 bg-green-500/10 px-2.5 py-1 text-[0.72rem] font-semibold text-green-500 whitespace-nowrap', dotClass: 'w-2 h-2 rounded-full flex-shrink-0 bg-green-400 shadow-[0_0_5px_rgba(74,222,128,0.5)]' };
    }
    return { label: 'Ocupado', icon: XCircle, badgeClass: 'inline-flex items-center gap-1 rounded-full border border-red-500/25 bg-red-500/[0.08] px-2.5 py-1 text-[0.72rem] font-semibold text-red-400 whitespace-nowrap', dotClass: 'w-2 h-2 rounded-full flex-shrink-0 bg-red-400 shadow-[0_0_5px_rgba(239,68,68,0.4)]' };
};
...
const eventosActivos = computed(() => eventos.value.filter(e => e.estado === 'free').length);
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-5 p-5">
            ...
            <div class="relative overflow-hidden rounded-[1.1rem] border border-green-400/[0.12] px-7 py-6"
                ...
                <div class="relative mb-5 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3.5">
                        <div
                            class="flex h-[46px] w-[46px] flex-shrink-0 items-center justify-center rounded-xl border border-green-400/30 bg-green-400/15 text-green-400">
                            <CalendarDays :size="24" />
                        </div>
                        <div>
                            <h1 class="text-[1.35rem] font-bold tracking-tight text-white"
                                style="letter-spacing:-0.03em;">Gestión de Eventos</h1>

                        </div>
                    </div>
                    <div
                        class="flex items-center gap-1.5 whitespace-nowrap rounded-full border border-green-400/30 bg-green-500/15 px-3 py-1.5 text-[0.76rem] font-semibold text-green-400">
                        <Sparkles :size="12" />
                ...
                <div class="relative flex items-center rounded-[10px] border border-green-400/[0.12] px-4 py-2.5"
                    ...
                    <div class="flex flex-1 items-center justify-center gap-2">
                        <div
                            class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-[7px] border border-green-500/25 bg-green-500/[0.18] text-green-400">
                            <Activity :size="14" />
                        </div>
                        <div>
                    ...
                    <div class="flex flex-1 items-center justify-center gap-2">
                        <div
                            class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-[7px] border border-emerald-500/25 bg-emerald-500/[0.18] text-emerald-400">
                    ...
                    <div class="flex flex-1 items-center justify-center gap-2">
                        <div
                            class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-[7px] border border-amber-500/25 bg-amber-500/[0.18] text-amber-400">
                            <DollarSign :size="14" />
                        </div>
                        <div>
                    ...
                    <div class="flex flex-1 items-center justify-center gap-2">
                        <div
                            class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-[7px] border border-blue-500/25 bg-blue-500/[0.18] text-blue-400">
                            <BarChart3 :size="14" />
                        </div>
                        <div>
                            <span class="block text-[0.95rem] font-bold leading-none text-white">{{ totalEventos -
                                eventosActivos }}</span>
                            <span class="mt-0.5 block text-[0.68rem]"
                                style="color:rgba(134,239,172,0.65);">Inactivos</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- ══ FORM CARD ════════════════════════════════════ -->
            <section
                class="relative overflow-hidden rounded-[1.1rem] border border-border bg-background transition-shadow duration-300 hover:shadow-[0_4px_24px_rgba(22,163,74,0.07)]">
                <!-- Stripe -->
                <div class="absolute inset-x-0 top-0 h-[2.5px]" :class="isEditing
                ...
                <div class="flex items-center gap-3.5 border-b border-border px-6 py-4"
                    ...
                    <div>
                        <h2 class="text-[0.95rem] font-bold tracking-tight text-foreground"
                ...
                <form class="grid grid-cols-1 gap-4 px-6 py-5 md:grid-cols-2" @submit.prevent="submit">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <Input v-model="form.nombre" type="text" required placeholder="Ej: Festival de Jazz 2025"
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="relative">
                            <span
                                class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[0.82rem] font-bold text-muted-foreground">$</span>
                            <Input v-model="form.precio" type="text" required placeholder="0.00"
                    ...
                    <div class="col-span-full flex flex-col gap-1.5">
                        ...
                        <Input v-model="form.descripcion" type="text" placeholder="Describe el evento brevemente..."
                            class="rounded-[9px] border-[1.5px] border-green-500/30 bg-zinc-800/50 text-[0.845rem] placeholder:text-muted-foreground/55 focus-visible:border-green-500 focus-visible:ring-2 focus-visible:ring-green-500/20" />
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <p class="mt-0.5 text-[0.68rem] text-muted-foreground/70">
                            <strong>Propio:</strong> el evento tiene su propia cancha/sala independiente. &nbsp;·&nbsp;
                            <strong>Compartido:</strong> varios eventos usan el mismo espacio físico.
                        </p>
                    ...
                    <div class="col-span-full flex items-center gap-2.5 pt-1">
                        ...
                        <button v-if="isEditing" type="button" :disabled="form.processing || deleteForm.processing"
            ...
            <section
                class="relative overflow-hidden rounded-[1.1rem] border border-border bg-background transition-shadow duration-300 hover:shadow-[0_4px_24px_rgba(22,163,74,0.07)]">
                <div
                    class="absolute inset-x-0 top-0 h-[2.5px] bg-gradient-to-r from-green-700 via-green-500 to-green-400" />

                <!-- Card header -->
                <div class="flex items-center gap-3.5 border-b border-border px-6 py-4"
                    style="background:rgba(39,39,42,0.30);">
                    <div
                        class="flex h-[38px] w-[38px] flex-shrink-0 items-center justify-center rounded-[9px] border border-blue-500/25 bg-blue-500/[0.08] text-blue-400">
                        <BarChart3 :size="16" />
                    </div>
                    <div>
                        <h2 class="text-[0.95rem] font-bold tracking-tight text-foreground"
                ...
                <table class="w-full border-separate border-spacing-0 text-[0.83rem]">
                    <thead>
                        <tr style="background:rgba(39,39,42,0.30);">
                            <th
                                class="w-[70px] whitespace-nowrap border-b border-green-500/15 px-4 py-2.5 text-left text-[0.68rem] font-bold uppercase tracking-[0.07em] text-green-500">
                                <Hash :size="11" class="mr-0.5 inline align-middle opacity-80" />ID
                            </th>
                            <th
                                class="whitespace-nowrap border-b border-green-500/15 px-4 py-2.5 text-left text-[0.68rem] font-bold uppercase tracking-[0.07em] text-green-500">
                            ...
                            <th
                                class="whitespace-nowrap border-b border-green-500/15 px-4 py-2.5 text-left text-[0.68rem] font-bold uppercase tracking-[0.07em] text-green-500">
                                <Link2 :size="11" class="mr-0.5 inline align-middle opacity-80" />Ambiente
                            </th>
                            <th
                                class="w-[160px] whitespace-nowrap border-b border-green-500/15 px-4 py-2.5 text-right text-[0.68rem] font-bold uppercase tracking-[0.07em] text-green-500">
                                Acciones
                            </th>
                        </tr>
                    </thead>
                        ...
                        <tr v-if="eventos.length === 0">
                            <td colspan="7" class="px-4 py-12">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    <div
                                        class="mb-2 flex h-16 w-16 items-center justify-center rounded-2xl border border-green-500/15 bg-green-500/[0.07] text-green-500 opacity-60">
                                        <CalendarDays :size="32" />
                                    </div>
                                    <p class="text-[0.9rem] font-bold text-muted-foreground">Sin eventos registrados</p>
                                    <span class="text-[0.77rem] text-muted-foreground opacity-60">Usa el formulario de
                                        arriba para crear tu primer evento</span>
                                </div>
                            </td>
                        </tr>

                        <!-- Filas -->
                        <tr v-for="events in eventos" :key="events.id"
                            ...
                            <td class="px-4 py-[0.85rem] align-middle">
                                <span
                                    class="inline-flex items-center rounded-md border border-green-500/[0.18] bg-green-500/[0.08] px-2 py-0.5 font-mono text-[0.72rem] font-bold text-green-500">
                                    #{{ events.id }}
                                </span>
                            </td>
                            ...
                            <td class="max-w-[220px] px-4 py-[0.85rem] align-middle">
                                <span v-if="events.descripcion"
                                    class="block overflow-hidden text-ellipsis whitespace-nowrap text-[0.8rem] text-muted-foreground">
                                    {{ events.descripcion }}
                                </span>
                            ...
                            <!-- Ambiente -->
                            <td class="px-4 py-[0.85rem] align-middle">
                                <span v-if="events.tipo_ambiente === 'compartido'"
                                    class="inline-flex items-center gap-1 rounded-full border border-blue-500/25 bg-blue-500/[0.08] px-2.5 py-1 text-[0.72rem] font-semibold text-blue-400 whitespace-nowrap">
                                    <Link2 :size="10" />
                                    Compartido · G{{ events.ambiente_grupo }}
                                </span>
                                <span v-else
                                    class="inline-flex items-center gap-1 rounded-full border border-zinc-500/20 bg-zinc-500/10 px-2.5 py-1 text-[0.72rem] font-semibold text-zinc-400 whitespace-nowrap">
                                    <Layers2 :size="10" />
                                    Propio
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-[0.85rem] align-middle text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button
                                        ...
                                        :disabled="form.processing || deleteForm.processing" @click="startEdit(events)"
